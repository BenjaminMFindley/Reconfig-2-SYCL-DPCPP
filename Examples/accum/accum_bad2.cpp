...
//
// See slides ADDLATER for a visual explanation of the strategy.
//
...
int main(int argc, char* argv[]) { 
  ...
  // Create the appropriate number of work-items.
  ...
  try {
	...
	handler.parallel_for<class accum>(cl::sycl::range<1> { num_work_items }, [=](cl::sycl::id<1> i) {
	    ...
	    for (int size = vector_size; size > 1; size = ceil(size / 2.0)) {
	      ...
	      // If the work-time is accessing the final element of an
	      // odd-sized array, just copy that odd element.
	      if (2*i + 1 == size) {
		x_d[i] = x_d[2*i];
	      }
	      // If the work-item is within the range of remaining elements,
	      // add a pair of elements. It is important to check this range
	      // because every iteration, only half as many work-items will
	      // have things to add. Although we could still let the extra
	      // work-items add without affecting correctness here, we also
	      // might want to protect against a user creating more work-items
	      // that would extend past the end of the vector and illegally
	      // access other portions of device memory.
	      else if (2*i + 1 < size)
