...
//
// This allowable error was chosen arbitrarily for this exmple. Normally, it would
...
bool are_floats_equal(float a, float b, float abs_tol=ALLOWABLE_ERROR, float rel_tol=ALLOWABLE_ERROR) {
  ...
  float diff = fabs(a-b);
  // If the diffrence is within the absolute tolerance, or the relative tolerance,
